...
- 🎨 **Красивый HTML-отчёт** — самодостаточная страница (дизайн от навыка `frontend-design`), которую не стыдно открыть на созвоне или отправить ссылкой. → [посмотреть пример](examples/sample-report.html)
...
Finalize   → финальная стратегия + executive-однопейджер; RICE считается в коде
Render     → навык frontend-design собирает красивый самодостаточный HTML-отчёт
```
